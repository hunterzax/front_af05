...
const TablePathDetail: React.FC<any> = ({ tableData, dataPeriod, setDataPeriod, dataPathDetailSearch, isReset, setIsReset, dataPathDetail, setDataPathDetail, selectedId, areaMaster }) => {
    ...
    const fetchPathDetail = async (data: any) => {
        ...
        setDataPeriod((prev: any) => {
            ...
            return (updatedDataXXX || [])?.map((item: any) => {
                return {
                    value: item?.period,
                    label: (item?.startDate ? formatDateNoTime(item.startDate) : '') + '-' + (item?.endDate ? formatDateNoTime(item.endDate) : '')
                };
            });
        });
    ...
    const calculateFields = (dataGroupArea: any[], areaName: any) => {
        ...
        (dataGroupArea || []).forEach((group: any) => {
            ...
            const preAvailList = (group?.data || []).map((item?: any) => Number(item?.adjust));
            let min_pre_avail = preAvailList.length > 0 ? Math.min(...preAvailList) : 0;

            const assignList = (group?.data || []).map((item?: any) => Number(item?.value));
            let min_assign = assignList.length > 0 ? Math.min(...assignList) : 0;

            const releaseList = (group?.data || []).map((item?: any) => Number(item?.release));
            let min_released = releaseList.length > 0 ? Math.max(...releaseList) : 0;
            ...
            if (areaName?.entry_exit?.name === "Entry") {
                ...
                const fDate = dataToUse?.startDate ? dayjs(dataToUse.startDate).format("DD/MM/YYYY") : ''
                ...
                filterVal?.map((e: any) => {
                    ...
                    let nmin_released_exit = (min_released_exit?.length ?? 0) > 0 ? Math.max(...min_released_exit) : 0
    ...
    useEffect(() => {
        if (dataToUse) {
                    ...
                    data: (dataToUse?.data || [])?.map((item: any) => {
                        ...
                        return {
                            ...
                            values: [
                                ...
                                calculatedFields?.nominated_value,
            ...
            const sortedDataTable = [
                {
                    data: [...(data_to_set?.[0]?.data || [])].sort((a, b) =>
                        (a?.area?.name || '').localeCompare(b?.area?.name || '')
                    )
                }
            ];

            const fullData = fillMissingAreas(sortedDataTable, areaMaster);
            setDataTableForGood(fullData);
        }
    }, [dataToUse, areaMaster]);
    ...
    useEffect(() => {
        if (dataPathDetail?.length > 0) {
            ...
            setDynamicColumns(data_filter_dynamic);
    ...
    const fillMissingAreas = (dataTableX: any[], areaMaster: any[]) => {
        if (!dataTableX || dataTableX.length === 0 || !dataTableX[0]) return dataTableX || [];
                                                                        ...
                                                                        title={
                                                                            <div>
                                                                                ...
                                                                                {(item?.path_display || [])?.map((pathGroup: any, groupIndex: number) => {
                                                                                    let npathGroup = []
                                                                                    for (let isort = 0; isort < (pathGroup?.length || 0); isort++) {
                                                                                        if (isort === 0) {
