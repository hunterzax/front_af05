...
const ChartSystem: React.FC<Props> = ({data}) => {
  ...
  useEffect(() => {
    ...
    const findHour: any = (useData || [])?.map((item: any) => {
      ...
      const date = item?.gas_day ? dayjs(item.gas_day).isValid() ? dayjs(item.gas_day).format('DD/MM/YYYY') : '' : ''
        ...
        switch (e?.type) {
          ...
          case 'line': {
            styleChart = {
              ...
              /**
               * สำคัญ:
               * ถ้าค่าเป็น null ไม่ให้เส้นขาด
               * ให้ลากข้ามไปต่อจุดถัดไป
               */
              spanGaps: true,
    ...
    plugins: {
      ...
      legend: {
        onClick: (e: any, legendItem: any, legend: any) => {
          ...
          if (!chart || datasetIndex === undefined || datasetIndex === null) return
              ...
              ?.map((item: any, i: any) => {
                ...
                if (item?.type == 'line') {
                  return {
                    datasetIndex: i,
                    text: `${item?.label}`,
                    hidden: chart?.isDatasetVisible ? !chart.isDatasetVisible(i) : false,
                    fillStyle: backgroundColor,
                    strokeStyle: backgroundColor,
                    fontColor: '#787486',
                    borderWidth: 1,
