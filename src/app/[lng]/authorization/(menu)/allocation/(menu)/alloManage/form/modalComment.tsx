...
const ModalComment: React.FC<FormExampleProps> = ({
    ...
    useEffect(() => {
        if (data) {
            ...
            setComments(filtered_comment.sort((a: any, b: any) => new Date(b?.create_date ?? 0).getTime() - new Date(a?.create_date ?? 0).getTime()))
            setCommentsShipper(filtered_comment.sort((a: any, b: any) => new Date(b?.create_date ?? 0).getTime() - new Date(a?.create_date ?? 0).getTime()))
            setCommentsReason(filtered_reason.sort((a: any, b: any) => new Date(b?.create_date ?? 0).getTime() - new Date(a?.create_date ?? 0).getTime()))
        }
    }, [data])
                    ...
                    <div className="flex flex-col items-center gap-2 p-9">
                        ...
                        <div className={`mb-1 w-[100%] ${comments?.length >= 2 ? 'max-h-[350px] overflow-y-auto' : ''}`}>
                            {(comments || [])?.map((item: any, index: number) => (
                                <div key={item?.id ?? index} className="w-full h-auto mb-2 p-2 border rounded-lg">
