            ...
            <div className="fixed inset-0 z-10 flex items-center justify-center">
                ...
                >
                    <div className="flex flex-col items-center gap-2 p-9 w-[700px]">
                        ...
                        <div className={`mb-1 w-[100%] ${(data?.release_capacity_submission_file?.length ?? 0) > 2 ? 'max-h-[350px] overflow-y-auto' : ''}`}>
                            {(data?.release_capacity_submission_file || [])?.map((item: any) => (
                                <div key={item?.id} className="w-full h-auto mb-2 p-2 border rounded-lg">
                                    <div className="flex flex-col p-2">
                                        <div className="mb-2 flex justify-between items-center">
                                            <span className="font-light">
                                                By <span className="font-bold text-[#58585A]">{item?.create_by_account ? `${item?.create_by_account?.first_name || ''} ${item?.create_by_account?.last_name || ''}` : ''}</span>
                                            </span>
