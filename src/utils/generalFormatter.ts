...
export const exportToExcelDailyAdjustReport = (data_current: any, data_filter: any, name: any, column?: any, extra_obj?: any) => {
  ...
  switch (name) {
    case 'tab-detail':
      ...
      let exportData2: any = (exportDataFilter?.length ?? 0) > 0 ? transformDailyAdjust2ForTable2(grouppppp, column) : []
...
//     let exportData: any = (exportDataFilter?.length ?? 0) > 0 ? transformDailyAdjustTabDetail(exportDataFilter, column, extra_obj?.displayUnit) : [];
...
export const exportToExcel = (data: any, name: any, column?: any, extra_obj?: any) => {
  let exportData = data
  // allocation-summary-shipper-report
  switch (name) {
    case 'group-2':
    ...
    case 'concept_point_limit':
      ...
      exportData = transformKeys(exportData)
      break
    case 'allocation-summary-shipper-report':
      // Exporting history for Area
      exportData = transformASR(data, column)
      exportData = transformKeys(exportData)
...
export const exportToXlsxWithNumericDetection = (exportData: any[], name = 'Export.xlsx', returnType: any = 'file') => {
  ...
  // 4) Header
  // =========================================================
  const headers: string[] = sheetData.length > 0 ? (sheetData[0] as string[]) : []
...
//       console.log('Excel cell too long:', {
//         row: rowIndex + 2, // +2 เพราะ Excel row 1 = header
//         key,
//         length: text.length,
...
export const exportShipperNomReportAll = (exportData?: any, name?: any, extra_obj?: any, returnType: any = 'file') => {
...
export const exportShipperNomReportView = (exportData?: any, name?: any, extra_obj?: any, returnType: any = 'file') => {
  // ที่ row 1 เพิ่ม 3 column 1. Gas Day 2.Shipper Name 3.Area
  // ที่ row 2 ใช้ข้อมูลจาก extra_obj เรียงตามนี้ 1.extra_obj.gas_day 2.extra_obj.shipper_name 3.extra_obj.area_text
...
export const exportCyberpunk = (data_to_export?: any) => {
  ...
  data_to_export?.forEach((entry: any) => {
    ...
    if(entry){
      row = [entry.gas_day]
      entryNomPoint = entry.nomPoint || []
    }
    pointHeaders.forEach(({point, shipper, type}: any) => {
      const foundPoint = entryNomPoint?.find((p: any) => p.point === point)
      if (!foundPoint) {
        row.push('')
        return
      }
      ...
      if (type === 'total') {
        ...
        const foundPointData = (foundPoint.data || [])
        const shipperData = foundPointData?.find((d: any) => d.shipper_name === shipper)
...
export const exportCyberpunk2 = (data?: any) => {
  ...
  data?.forEach((entry: any) => {
    entry?.nomPoint?.forEach((point: any) => {
      if (point?.point && !points[point.point]) points[point.point] = new Set()
  ...
  data?.forEach((entry: any) => {
    ...
    let rowTotal = 0
    const entryNomPoint = (entry.nomPoint || [])

    orderedPoints.forEach((point) => {
      const pointEntry = entryNomPoint.find((p: any) => p.point === point)
...
export const exportCyberpunk4 = async (data?: any) => {
  ...
  data?.forEach((entry: any) => {
    entry?.nomPoint?.forEach((point: any) => {
      if (point?.point && !points[point.point]) points[point.point] = new Set()
      point?.data?.forEach((d: any) => { if (point?.point) { if (point?.point && !points[point.point]) points[point.point] = new Set(); points[point.point].add(d?.shipper_name); } })
    })
  })
...
export const mergeDataByGroupMedTerm = (data_med_term_each?: any) => {
  ...
  data_med_term_each?.forEach((entry: any) => {
    ...
    entry?.data?.forEach((newData: any) => {
      ...
      if (existingData) {
        existingData.value = (existingData?.value || []).map((val: any, idx: any) => (val ?? 0) + (newData?.value?.[idx] ?? 0))
      } else {
...
export const sumDataNomShipperReportOLD = (data_for_sum: any[]) => {
  ...
  for (const item of data_for_sum) {
    ...
    for (const key of keysToSum) {
      ...
      if (existing?.data_temp) { existing.data_temp[key] = fmt3(existingValue + currentValue) }
    }
...
export const sumDataNomShipperReportNo = (data_for_sum: any[]) => {
  ...
  grouped.forEach((existing, groupKey) => {
     ...
     */
    if (existing?.data_temp) {
      const dt: any = existing.data_temp;
      for (const key of keysToSum) {
        if (dt) dt[key] = fmt3(toNumber(dt?.[key]));
      }
    }
...
export const splitByGroup = (data_origin?: any) => {
  ...
  for (const entry of (data_origin || [])) {
    ...
    if (Array.isArray(entry?.gasWeek)) {
...
export const sortRevisedCapacityPathBlocks = (blocks: AreaBlock[]): AreaBlock[] => {
  return (blocks ?? []).map((block) => ({
    ...
    pathConfigs: (block.pathConfigs ?? []).map((cfg) => {
      ...
      for (const it of (items || [])) {
        if (it?.area_id != null && !orderAreaIds?.includes(it?.area_id)) orderAreaIds.push(it?.area_id)
      }
...
export const addSumPerRow = (data_new?: any) => {
  const addSumPerRow = (data_new ?? []).map((row: any) => {
    ...
    let ndataRow = (dataRow ?? []).map((e_: any) => {
      ...
      let overusageMMBTUD_ = (_nE?.capacityRightMMBTUD ?? 0) - (_nE?.nominatedValueMMBTUD ?? 0)
...
export const exportToExcelAllShipperNominationReport = (arr:any) => {
  console.log('arr : ', arr);

  const sheets = arr?.map((ex:any, ix:any) => {
    const { typeExport, nameSheet, data, config} = ex
    const { columnVisibility, type, extra_obj} = config
    let sheet_ = null
    let exportData = data
    switch (type) {
      case 'shipper-nom-report-view':
        exportData = transformShipperNomReportView(data, columnVisibility, extra_obj)
        exportData = transformKeys(exportData)
        const keyMapShipperNomReportView: Record<string, string> = {
          'Capacity Right Mmbtud': 'Capacity Right (MMBTU/D)',
          'Nominated Value Mmbtud': 'Nominated Value (MMBTU/D)',
        ...
        break
      case 'shipper-nom-report-detail':
        exportData = transformShipperNomReportDetail(data, columnVisibility, extra_obj)
        exportData = transformKeys(exportData)
        const keyMapBalx: Record<string, string> = {
          'Week Day': `${extra_obj?.day_text}-${extra_obj?.date}`,
          'Concept Id': 'Concept ID'
        ...
        break
      case 'shipper-nom-report-tab-0':
        exportData = transformShipperNomReportTabZero(data, columnVisibility)
        exportData = transformKeys(exportData)
        break
      case 'shipper-nom-report-tab-weekly':
        exportData = transformShipperNomReportTabWeekly(data, columnVisibility, extra_obj)
        exportData = transformKeys(exportData)
        break
      default:
        exportData = data
    ...
    if (type == 'shipper-nom-report-detail') {
      sheet_ = exportShipperNomReportAll(exportData, nameSheet, extra_obj, "sheet")
    } else if (type == 'shipper-nom-report-view') {
      sheet_ = exportShipperNomReportView(exportData, nameSheet, extra_obj, "sheet")
    } else {
      sheet_ = exportToXlsxWithNumericDetection(
        exportData,
        nameSheet,
        "sheet"
      )
  ...
  })?.filter((item: any) => item?.sheet)

  if (!sheets?.length) return
  const workbook = XLSXStyle.utils.book_new()
  sheets.forEach((item: any, index: number) => {
    XLSXStyle.utils.book_append_sheet(
      workbook,
      item.sheet,
      item.nameSheet || `Sheet${index + 1}`
    )
  })
  XLSXStyle.writeFile(
    workbook,
    'Shipper Nomination Report All.xlsx'
  )
}
